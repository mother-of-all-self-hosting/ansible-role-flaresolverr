...
#
# Tags look like `v<FlareSolverr version>-<release>`, which is what this
# repository has always published (v3.4.1-0 ... v3.5.0-2):
#
# - if defaults/main.yml points at a FlareSolverr version that has never been
#   released, the release counter restarts at 0 (`v3.5.1-0`)
# - otherwise the counter is incremented (`v3.5.1-1`), but only if something
...
#
# The commit-message approach this replaced scanned the last 20 commit subjects
# for one authored by renovate[bot] that mentioned both "docker tag to " and
# "flaresolverr". Every release that did not arrive that way went untagged, and
# a bump that fell out of that 20-commit window went untagged as well.
...
# Anchored on `flaresolverr_version:` so that neither a commented-out example
# nor `flaresolverr_container_image_tag`, which is derived from it, can be
# mistaken for it.
version="$(sed -nE 's|^flaresolverr_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the FlareSolverr version from $defaults_path"
	exit 1
fi

# FlareSolverr's own versions carry a leading `v` (`v3.5.0`) and so do this
# repository's tags, so the `v` must not be doubled. Tolerate a value without
# one, so that a future change of upstream convention does not produce tags
# that no longer sort next to the existing ones.
